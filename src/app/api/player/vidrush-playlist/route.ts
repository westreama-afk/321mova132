...
const PROVIDERS: ProviderDef[] = [
  ...
  },
  {
  ...
  },
   {
    ...
    label: "mega",
    encrypted: true,
    movieUrl: "https://api.tulnex.com/provider/vidlink/movie/${id}",
    tvUrl: "https://api.tulnex.com/provider/vidlink/tv/${id}/${season}/${episode}",
  },
...
const fetchProviderSources = async (
  ...
  try {
    ...
    return extractStreams(data, provider.label, defaultHeaders);
  } catch {
    return [];
  }
};
...
export const GET = async (request: NextRequest) => {
  const req = parseMediaRequest(request.nextUrl.searchParams);
  if (!req) return NextResponse.json({ error: "Invalid parameters" }, { status: 400 });

  const results = await Promise.allSettled(PROVIDERS.map((p) => fetchProviderSources(p, req)));
