...
export interface UnifiedPlayerEventData {
  ...
  playerSource?: "321movies" | "external";
}
...
interface LocalPlayerEventData { event: PlayerEventType; currentTime: number; duration: number; mediaId: string | number; mediaType: ContentType; season?: number; episode?: number; progress?: number; playerSource?: "321movies" | "external" }
...
export function usePlayerEvents(options: UsePlayerEventsOptions = {}) {
  ...
  const normalizePayload = (data: UnifiedPlayerEventData): UnifiedPlayerEventData => ({ ...data, mediaId: media?.id ?? data.mediaId ?? 0, mediaType: media?.type ?? data.mediaType ?? "movie", season: metadata?.season ?? data.season ?? 0, episode: metadata?.episode ?? data.episode ?? 0, activeWatchSeconds: data.activeWatchSeconds ?? activeWatchSecondsRef.current, playerSource: data.playerSource ?? "external" });
